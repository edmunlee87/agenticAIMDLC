"""stresssdk.sdk -- StressSDK: concrete domain SDK for Stress Testing models."""
...
_MANIFEST_PATH = Path(__file__).parent.parent.parent.parent.parent.parent / "configs" / "runtime" / "domain_overlays" / "stress.yaml"


class StressSDK(BaseDomainSDK):
    """Domain SDK for Stress Testing models."""
    ...
    def get_manifest(self) -> DomainPackManifest:
        """Return the Stress Testing domain pack manifest."""
        if self._manifest is None:
            self._manifest = self._loader.load("stress", self._manifest_path)
        return self._manifest

    def compute_stage(self, stage_id: str, inputs: dict[str, Any]) -> StageComputeResult:
        """Execute Stress Testing domain computation.
