"""lgdsdk.sdk -- LGDSDK: concrete domain SDK for LGD models."""
...
_MANIFEST_PATH = Path(__file__).parent.parent.parent.parent.parent.parent / "configs" / "runtime" / "domain_overlays" / "lgd.yaml"


class LGDSDK(BaseDomainSDK):
    """Domain SDK for LGD models."""
    ...
    def get_manifest(self) -> DomainPackManifest:
        """Return the LGD domain pack manifest."""
        if self._manifest is None:
            self._manifest = self._loader.load("lgd", self._manifest_path)
        return self._manifest

    def compute_stage(self, stage_id: str, inputs: dict[str, Any]) -> StageComputeResult:
        """Execute LGD domain computation.
