"""sicr_sdk.sdk -- SICRSDK: concrete domain SDK for SICR models."""
...
_MANIFEST_PATH = Path(__file__).parent.parent.parent.parent.parent.parent / "configs" / "runtime" / "domain_overlays" / "sicr.yaml"


class SICRSDK(BaseDomainSDK):
    """Domain SDK for SICR models."""
    ...
    def get_manifest(self) -> DomainPackManifest:
        """Return the SICR domain pack manifest."""
        if self._manifest is None:
            self._manifest = self._loader.load("sicr", self._manifest_path)
        return self._manifest

    def compute_stage(self, stage_id: str, inputs: dict[str, Any]) -> StageComputeResult:
        """Execute SICR domain computation.
