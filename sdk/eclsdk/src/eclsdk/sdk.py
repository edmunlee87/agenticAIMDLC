"""eclsdk.sdk -- ECLSDK: concrete domain SDK for ECL models."""
...
_MANIFEST_PATH = Path(__file__).parent.parent.parent.parent.parent.parent / "configs" / "runtime" / "domain_overlays" / "ecl.yaml"


class ECLSDK(BaseDomainSDK):
    """Domain SDK for ECL models."""
    ...
    def get_manifest(self) -> DomainPackManifest:
        """Return the ECL domain pack manifest."""
        if self._manifest is None:
            self._manifest = self._loader.load("ecl", self._manifest_path)
        return self._manifest

    def compute_stage(self, stage_id: str, inputs: dict[str, Any]) -> StageComputeResult:
        """Execute ECL domain computation.
